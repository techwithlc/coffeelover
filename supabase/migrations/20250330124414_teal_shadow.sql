/*
  ...
  1. Tables
    - `locations` with user_id column
    - `reviews` with user_id column
    - `favorites` with user_id column
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'favorites') THEN
    CREATE TABLE favorites (
      ...
      CONSTRAINT favorites_location_id_fkey FOREIGN KEY (location_id) REFERENCES locations(id) ON DELETE CASCADE,
      CONSTRAINT favorites_user_location_unique UNIQUE(user_id, location_id)
    );
  END IF;
END $$;
